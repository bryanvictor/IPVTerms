...
variables.to.keep <- c("KW", "KP", "AD", "AF", "TI", "AU", "SO", "YR", "AB", "LO", "S2", 
                       "AF", "PY", "JN", "PD", "DT")
...
####This code pulls the year out of the SO field for a number of records

SO<-filter(DF, attributes=="SO")

SO.temp1<-filter(SO, grepl("[:alpha:]*\\(.", record))

SO.temp1$record<-gsub(" ", "", SO.temp1$record)

SO.temp1$record<-sub("^([^,]*\\([^,]*\\),?[[:digit:]]{4}).*", "\\1", SO.temp1$record)
SO.temp1$record<-gsub("^[^,]*\\([^,]*\\),?", "", SO.temp1$record)
SO.temp1$record<-gsub("^.*\\),", "", SO.temp1$record)
SO.temp1$record<-sub("^([[:digit:]]{4}).*","\\1", SO.temp1$record)
SO.temp1<-filter(SO.temp1, grepl("[[:digit:]]{4}", record))
SO.temp1<-mutate(SO.temp1, attributes=ifelse(attributes=="SO", "YR", NA))

DF<-rbind(DF, SO.temp1)


    DF$attributes <- ifelse(DF$attributes == "PY|DT", "YR", DF$attributes)
